...
export const CalendarModal = () => {
    ...
    const tittleClass = useMemo(() => {
        ...
        return ( formValues.title.length > 3 ) //Minimo de letras para que acepte el título
            ? 'is-valid'
            : 'is-invalid';
    ...
    const onSubmit = async ( event: any ) =>{
        ...
        if (isNaN(difference) || difference <= 0 || difference < 3600) {
            Swal.fire('Error al selecionar fechas', 'Verificar las fechas', 'error');
            return;
        }

        if ( formValues.title.length <= 3 ) return; //Minimo de letras para que pase el título
